...
describe('constructUrl', () => {
  ...
  it('should construct URL with all params', () => {
    ...
    const result: UrlResult = constructUrl(params);
    expect(result.url).toBe('http://example.com/auth?client_id=client&redirect_uri=http%3A%2F%2Flocalhost&scope=openid&response_type=code+token+id_token&response_mode=query&state=state&nonce=nonce&prompt=login');
  });
