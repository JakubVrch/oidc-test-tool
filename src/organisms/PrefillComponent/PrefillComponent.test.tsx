...
describe("PrefillComponent", () => {
  beforeEach(() => {
    render(
      <Prefill onPrefill={mockOnPrefill} prefillConfig={mockPrefillConfig} />,
    );
  });
  ...
  test("Prefill component renders correctly", () => {
    expect(
      screen.getByRole("combobox", { name: /Select Config:/i }),
    ).toBeInTheDocument();
  ...
  test("Prefill component shows description", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText("Config 1", { selector: '[data-part="value-text"]' }),
      ).toBeInTheDocument();
