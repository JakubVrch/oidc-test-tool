...
describe("ConstructRequestForm", () => {
  ...
  it("renders the form fields", () => {
    ...
    expect(
      screen.getByRole("combobox", { name: /Response Mode/i }),
    ).toBeInTheDocument();
  ...
  it("shows error messages for missing required fields", async () => {
    ...
    await waitFor(() => {
      const errorMessagesRequired = screen.getAllByText(
        /This field is required/i,
      );
      expect(errorMessagesRequired.length).toBe(4);
      expect(
        screen.getByText(/At least one response type is required/i),
      ).toBeInTheDocument();
    });
  });
  ...
  it("constructs URL based on minimal form input changes", async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByText(
          /http:\/\/example\.com\/auth\?client_id=client&redirect_uri=http%3A%2F%2Flocalhost&scope=openid&response_type=code/i,
        ),
      ).toBeInTheDocument();
    });
  });
  ...
  it("submits the form with optional parameters", async () => {
    ...
    await userEvent.type(
      screen.getByLabelText(/Token Endpoint/i),
